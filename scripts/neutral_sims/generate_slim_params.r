...
n_ne <- 2e3
n_final <- 13333
n_mut <- 0.001 #0.000367994 #sample(pi_dist[[1]], 1, replace=TRUE) / (4*n_ne)#divide pi_dist by 4*N_e. b/c per nucleotide
...
#bneck_file <- paste("results/neutral_sims/bneck.", JOB_ID, ".", ITER, ".txt", sep="")
#cat(n_bneck_size, "\n", file=bneck_file)
...
paramsFile <- paste("results/neutral_sims/params_file.", JOB_ID, ".", ITER, ".txt", sep="")
cat(command, "\n", file=paramsFile)  #will need to pass task id variable
system(paste("/home/adurvasu/software/slim ", paramsFile, sep=""))
